  ...
  if (!results)
    return (
      <p style={{ color: "#444", fontSize: "0.82rem" }}>
        Results available once election starts.
      </p>
    );
    ...
    <div>
      <div style={s.meta}>
        <div style={s.metaItem}>
          <span style={s.metaLabel}>Total Votes</span>
          <span style={s.metaVal}>{results.total_votes}</span>
        </div>
        <div style={s.metaItem}>
          <span style={s.metaLabel}>Eligible</span>
          <span style={s.metaVal}>{results.total_eligible}</span>
        </div>
        <div style={s.metaItem}>
          <span style={s.metaLabel}>Turnout</span>
        ...
        {results.winner && (
          <div style={s.metaItem}>
            <span style={s.metaLabel}>Winner</span>
            <span style={{ ...s.metaVal, color: "#facc15" }}>
              {results.winner}
            </span>
          </div>
        )}
      ...
      {results.tallied_at && (
        <p style={s.timestamp}>
          Last tallied: {new Date(results.tallied_at).toLocaleString()}
        </p>
      )}
    </div>
  );
