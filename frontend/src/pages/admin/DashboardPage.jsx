    ...
    <div style={s.root}>
      ...
      <div style={s.container}>
        <div style={s.hero}>
          ...
          <p style={s.sub}>
            Manage elections, candidates, and voter eligibility.
          </p>
        </div>
        ...
        <div style={s.statsRow}>
          {["draft", "active", "completed"].map((st) => (
            <div key={st} style={s.statCard}>
              <span style={{ ...s.statNum, color: statusColor[st] }}>
                {elections.filter((e) => e.status === st).length}
              </span>
        ...
        {loading ? (
          ...
          <div style={s.empty}>
            ...
            <button style={s.newBtnLg} onClick={() => setShowModal(true)}>
              ...
              <tbody>
                {elections.map((e) => (
                  <tr key={e.id} style={s.tr}>
                    <td style={s.td}>
                      <span style={s.elTitle}>{e.title}</span>
                    </td>
                    <td style={s.td}>
                      <span
                        style={{
                          ...s.statusBadge,
                          color: statusColor[e.status] || "#555",
                          borderColor: statusColor[e.status] || "#333",
                        }}
                      >
                        {e.status.toUpperCase()}
                      </span>
                    </td>
                    <td style={{ ...s.td, color: "#444", fontSize: "0.72rem" }}>
                      {new Date(e.created_at).toLocaleDateString()}
                    </td>
                    <td style={s.td}>
                      <button
                        style={s.manageBtn}
                        onClick={() => navigate(`/admin/election/${e.id}`)}
                      >
                        Manage →
                      </button>
                    </td>
          ...
          <div style={s.modal}>
            ...
            <form onSubmit={handleCreate} style={s.form}>
              ...
              <div style={s.btnRow}>
                ...
                <button type="submit" style={s.greenBtn} disabled={creating}>
