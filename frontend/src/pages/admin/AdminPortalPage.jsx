  ...
  async function submit(e) {
    ...
    try {
      ...
      if (res.data.role !== "admin") {
        setError(
          "This account does not have admin access. Use the Student portal.",
        );
        return;
      }
...
function AdminCreateForm({ navigate }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      setSuccess(
        `Admin account created for ${register_number.toUpperCase()}. You can now login.`,
      );
      setForm({
        register_number: "",
        full_name: "",
  ...
  if (success) {
    return (
      <div style={s.successBox}>
        ...
        <button style={s.btnYellow} onClick={() => setSuccess("")}>
          ← Back to Login
        </button>
      </div>
    );
  ...
  return (
    <form onSubmit={submit} style={s.form}>
      <div style={s.infoNote}>
        ⚠ Admin registration requires an{" "}
        <strong style={{ color: "#facc15" }}>invite key</strong> from your
      ...
      <input
        style={s.input}
        type="password"
        placeholder="Secret key from system admin"
        value={form.invite_key}
        onChange={(e) => update("invite_key", e.target.value)}
      />
