...
export default function ActiveElectionsPage() {
  ...
  useEffect(() => {
    getActiveElections().then(r => setElections(r.data)).catch(() => {}).finally(() => setLoading(false));
  }, []);
    ...
    <div style={s.root}>
      ...
      <div style={s.container}>
        <div style={s.hero}>
          ...
          <p style={s.sub}>Cast your vote securely. Every ballot is anonymous and tamper-proof.</p>
        </div>
          ...
          <div style={s.grid2}>
            {elections.map(e => (
              <div key={e.id} style={s.card}>
                ...
                <button style={s.voteBtn} onClick={() => navigate(`/voter/ballot/${e.id}`)}>
                  Cast Your Vote →
                </button>
              </div>
            ))}
...
const s = {
  ...
  grid: { position: "fixed", inset: 0, backgroundImage: "linear-gradient(rgba(74,222,128,0.03) 1px,transparent 1px),linear-gradient(90deg,rgba(74,222,128,0.03) 1px,transparent 1px)", backgroundSize: "40px 40px", pointerEvents: "none", zIndex: 0 },
  container: { maxWidth: "900px", margin: "0 auto", padding: "48px 24px", position: "relative", zIndex: 1 },
  ...
  card: { background: "#111", border: "1px solid #1e1e1e", borderRadius: "12px", padding: "28px" },
  cardTop: { display: "flex", alignItems: "center", gap: "8px", marginBottom: "14px" },
  liveDot: { width: "6px", height: "6px", borderRadius: "50%", background: "#4ade80", boxShadow: "0 0 6px #4ade80" },
  liveText: { fontSize: "0.6rem", color: "#4ade80", letterSpacing: "0.15em" },
  cardTitle: { fontSize: "1.05rem", fontWeight: "700", color: "#fff", margin: "0 0 8px" },
  cardDesc: { fontSize: "0.78rem", color: "#555", margin: "0 0 20px", lineHeight: "1.5" },
  voteBtn: { background: "#4ade80", color: "#0a0a0a", border: "none", borderRadius: "8px", padding: "12px 20px", fontSize: "0.85rem", fontWeight: "700", fontFamily: "inherit", cursor: "pointer", width: "100%" },
};
